...
const OPPORTUNITY_RE = /consult|tender|procurement|expression of interest|\beoi\b|request for proposal|\brfp\b|request for quotation|\brfq\b|bid|grant|funding|vacanc|job|fellowship|call for proposal|technical assistance|framework contract|roster|invitation to tender/i;
...
export class PublicPageScraper extends BaseScraper {
  ...
  private inferType(text: string): RawOpportunity['type'] { const lower=text.toLowerCase(); if (/consultant|consultancy|technical assistance|advisor|advisory/.test(lower)) return 'consultancy'; if (/tender|procurement|expression of interest|\beoi\b|request for proposal|\brfp\b|request for quotation|\brfq\b|invitation for bids?|framework contract/.test(lower)) return 'tender'; if (/grant|funding|call for proposals|challenge fund/.test(lower)) return 'grant'; if (/fellowship|scholarship|training/.test(lower)) return 'fellowship'; return 'job'; }
