...
import {
  ...
  Clock3, ShieldCheck, CircleDollarSign, UsersRound, MessageSquareText,
  CheckCircle2, ChevronRight, RefreshCw, X, Menu, Target, Layers3, FileCheck2, Send,
  Upload, Trash2, CircleGauge, CalendarDays, Globe2, Building2, Check,
...
const VIEW_META = {
  ...
  saved: { label: 'Shortlist', icon: BookmarkCheck },
...
function TukuAIIcon({ size = 16, className = '' }) {
  return <img src="/brand/tuku-ai.svg" alt="" aria-hidden="true" width={size} height={size} className={cx('tuku-ai-icon', className)} style={{ width: size, height: size, objectFit: 'contain', flex: '0 0 auto' }} />;
}
...
function App() {
  ...
  async function loadOpportunities(next = filters, options = {}) {
    ...
    try {
      ...
      } else setOpportunities(items);
      setPageInfo(data.pageInfo || { nextCursor: null, hasMore: false });
    } catch (error) { notify(error.message); }
    finally { append ? setLoadingMore(false) : setOpportunityLoading(false); }
  }
  async function openOpportunity(id) {
    if (!id) return notify('This Radar item is missing its opportunity reference.');
    try { setSelected(await api(`/api/opportunities/${encodeURIComponent(id)}`)); }
    catch (error) { notify(error.message); }
  }
  async function openOpportunityAndShow(id) {
    if (!id) return;
    setView('discover');
    await openOpportunity(id);
  }
  async function openWorkspaceAndShow(id) {
    if (!id) return;
  ...
  useEffect(() => {
    ...
    const token = params.get('recoveryToken') || params.get('recovery_token') || params.get('token') || '';
    const reset = params.get('reset_password') === '1' || Boolean(token);
    if (reset) { setRecoveryToken(token); setResetRequested(true); setAuthOpen(true); }
    else if (params.get('workspace')) setAuthOpen(true);
  }, []);
  useEffect(() => { const timer = setTimeout(() => { setSelected(null); loadOpportunities(filters); }, 280); return () => clearTimeout(timer); }, [filters]);
  useEffect(() => {
    if (view === 'saved' && session) loadSaved();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (workspaceId) await openWorkspaceAndShow(workspaceId);
        else if (opportunityId) await openOpportunityAndShow(opportunityId);
        setAuthOpen(false);
        params.delete('workspace'); params.delete('opportunity');
  ...
  async function addMember(email, role) {
    try {
      const result = await api(`/api/workspaces/${workspace.id}/members`, { method: 'POST', body: JSON.stringify({ email, role }) });
      await refreshWorkspace();
      notify(result.delivery?.delivered ? 'Collaborator added and invitation email sent.' : 'Collaborator added. Email delivery could not be confirmed.');
    } catch (error) { notify(error.message); }
  }
  ...
  async function resolveComment(commentId, status = 'resolved') {
    try { await api(`/api/workspaces/${workspace.id}/comments/${commentId}`, { method: 'PATCH', body: JSON.stringify({ status }) }); await refreshWorkspace(); notify(status === 'resolved' ? 'Review comment resolved.' : 'Review comment reopened.'); }
    catch (error) { notify(error.message); }
  }
  async function summarizeTeamReview() {
    try { const data = await api(`/api/workspaces/${workspace.id}/ai/team-summary`, { method: 'POST', body: '{}' }); return data.text || 'No unresolved team actions found.'; }
  ...
  async function loadSaved() { try { const data = await api('/api/me/saved'); setSavedItems(data.items || []); } catch (error) { notify(error.message); } }
  ...
  async function updateApplication(opportunityId, status, notes = '') {
    if (!opportunityId || !requireAuth()) return;
    try {
      await api(`/api/opportunities/${opportunityId}/applications`, { method: 'POST', body: JSON.stringify({ status, notes }) });
      await Promise.all([loadApplications(), loadPriorityQueue()]);
  ...
  async function toggleSave() {
    ...
    try {
      ...
      setSelected((old) => ({ ...old, saved: result.saved })); if (session) await loadSaved(); notify(result.saved ? 'Saved to shortlist.' : 'Removed from shortlist.');
    } catch (error) { notify(error.message); }
  }
  ...
  async function updateLibraryDocument(id, payload) {
    try { const data = await api(`/api/me/documents/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }); await loadDocuments(); notify(payload.reviewed ? 'Evidence document saved and verified.' : 'Evidence document updated.'); return data; }
  ...
  async function saveProfile(payload) { try { const p = await api('/api/me/profile', { method: 'PUT', body: JSON.stringify(payload) }); setProfile(p); await loadSession(); notify('Matching profile updated.'); await loadOpportunities(); } catch (error) { notify(error.message); } }
  ...
  async function previewBriefing(payload = {}) { setBriefPreviewLoading(true); try { const data = await api('/api/me/briefing/preview', { method: 'POST', body: JSON.stringify(payload) }); setBriefPreview(data.items || []); } catch (error) { notify(error.message); } finally { setBriefPreviewLoading(false); } }
  ...
  return <div className="radar-app">
    ...
    <div className="app-stage">
      ...
      <main className="page-stage">
        <AnimatePresence mode="wait">
          ...
          {view === 'documents' && <motion.div key="documents" {...pageMotion}><DocumentsView session={session} documents={documents} evidenceHealth={evidenceHealth} addDocument={addLibraryDocument} updateDocument={updateLibraryDocument} deleteDocument={deleteDocument} extractEvidence={extractEvidence} notify={notify} onAuth={() => setAuthOpen(true)} /></motion.div>}
    ...
    <AuthModal open={authOpen} setOpen={setAuthOpen} mode={authMode} setMode={setAuthMode} recoveryToken={recoveryToken} resetRequested={resetRequested} onResetComplete={() => { setRecoveryToken(''); setResetRequested(false); const u = new URL(window.location.href); ['reset_password','recoveryToken','recovery_token','token'].forEach((key) => u.searchParams.delete(key)); window.history.replaceState({}, '', `${u.pathname}${u.searchParams.toString() ? `?${u.searchParams.toString()}` : ''}`); setAuthOpen(false); }} onSuccess={async () => { setAuthOpen(false); await loadSession(); notify('Welcome to Radar.'); }} notify={notify} />
    ...
    <AnimatePresence>{toast && <motion.div className="toast" role="status" aria-live="polite" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}>{toast}</motion.div>}</AnimatePresence>
  </div>;
}

const pageMotion = { initial: { opacity: 0, y: 8 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -4 }, transition: { duration: .22 } };

function Sidebar({ view, setView, session, mobileNav, setMobileNav, trialDays, onNewSearch }) {
  return <>
    ...
    <aside className={cx('sidebar', mobileNav && 'open')}>
      ...
      <button className="new-search-btn" onClick={() => { onNewSearch(); setMobileNav(false); }}><Plus size={18} /> New search</button>
      ...
      <div className="sidebar-card">{session ? <TukuAIIcon size={18} /> : <Sparkles size={18} />}<div><strong>{session ? 'Radar AI is ready' : 'Unlock your Radar'}</strong><small>{session ? 'Ask from any opportunity or workspace.' : 'Sign in to match and prepare applications.'}</small></div></div>
      <div className="sidebar-foot"><span className="pulse-dot" /> Verified opportunity feed</div>
    </aside>
  </>;
}

function Topbar({ filters, onSearch, session, onAuth, onLogout, unread, onNotifications, onAI, onMobile }) {
  return <header className="topbar">
    <button className="mobile-menu" onClick={onMobile} aria-label="Open navigation"><Menu /></button>
    <div className="global-search"><Search size={19} /><input aria-label="Search opportunities" value={filters.q} onChange={(e) => onSearch(e.target.value)} placeholder="Search opportunities, organisations, countries…" /></div>
    <div className="top-actions"><button className="icon-btn ai-btn" onClick={onAI} aria-label="Open Radar AI"><TukuAIIcon size={18} /></button><button className="icon-btn" onClick={onNotifications} aria-label="Open notifications"><Bell size={18} />{unread > 0 && <b>{unread > 9 ? '9+' : unread}</b>}</button>{session ? <div className="account-pill"><div className="avatar">{(session.user?.name || session.user?.email || 'R').slice(0,1).toUpperCase()}</div><div><strong>{session.user?.name || 'Radar user'}</strong><small>{session.user?.email}</small></div><button onClick={onLogout}><LogOut size={16} /></button></div> : <button className="button dark" onClick={onAuth}>Sign in</button>}</div>
  </header>;
}
...
function OpportunityPanel({ row, onClose, onSave, onAI, onWorkspace, onTrack, busy, session, onAuth }) {
  ...
  return <><motion.button className="detail-scrim" aria-label="Close opportunity details" onClick={onClose} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}/><motion.aside className="opportunity-panel detail-drawer" role="dialog" aria-modal="true" aria-label={`Opportunity details: ${row.title}`} initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={spring}><button className="drawer-close" onClick={onClose} aria-label="Close"><X size={18}/></button><div className="detail-scroll"><div className="detail-kicker"><span className={cx('trust-pill', trustTone)}><span />{trust}</span><span>{row.type || 'Opportunity'}</span></div><h2>{row.title}</h2><p className="detail-org"><Building2 size={15}/>{row.organization || 'Organisation'} · {row.country || 'Location not listed'}</p>
    ...
    <div className="application-actions">
      {safeUrl(row.applicationUrl) ? <a className="button apply-primary" href={safeUrl(row.applicationUrl)} target="_blank" rel="noreferrer"><ExternalLink/> Apply now</a> : row.applicationEmail ? <a className="button apply-primary" href={`mailto:${row.applicationEmail}?subject=${encodeURIComponent(row.title || 'Application')}`}><Mail/> Apply by email</a> : <button className="button application-unresolved" disabled><AlertTriangle/> Application route not resolved</button>}
      <button className="button gold" onClick={onWorkspace} disabled={busy === 'workspace-create'}>{busy === 'workspace-create' ? <LoaderCircle className="spin"/> : <BriefcaseBusiness/>}{row.workspace ? 'Open Radar workspace' : 'Prepare in Radar'}</button>
      <button className="button ghost" onClick={onSave}>{row.saved ? <BookmarkCheck/> : <Bookmark/>}{row.saved ? 'Saved' : 'Save'}</button>
    </div>
    {row.applicationInstructions && <InfoBox tone="teal" icon={Send} title="How to apply" text={row.applicationInstructions} />}
    <div className="ai-action-grid"><button onClick={() => onAI('eligibility')} disabled={busy === 'opp-eligibility'}>{busy === 'opp-eligibility' ? <LoaderCircle className="spin"/> : <TukuAIIcon size={17}/>}<span><strong>Check eligibility</strong><small>Met, gaps & blockers</small></span></button><button onClick={() => onAI('brief')} disabled={busy === 'opp-brief'}>{busy === 'opp-brief' ? <LoaderCircle className="spin"/> : <TukuAIIcon size={17}/>}<span><strong>Prepare brief</strong><small>Requirements + next action</small></span></button></div>
    ...
    {row.aiResult && <div className="ai-result"><div><TukuAIIcon size={17}/><strong>Radar AI</strong></div><p>{row.aiResult}</p></div>}
    ...
    <DetailSection title="Source & provenance">{source ? <a className="source-link" href={source} target="_blank" rel="noreferrer"><ShieldCheck/><span><strong>{row.source || 'Original source'}</strong><small>View the page Radar used to verify this opportunity</small></span><ExternalLink/></a> : <p>Source URL unavailable.</p>}{!row.applicationUrl && !row.applicationEmail && <p className="application-source-note">Radar has not yet resolved a direct application form or application email. The source above is for verification and should not be treated as the application destination.</p>}</DetailSection>
...
function SavedView({ session, items, selected, openOpportunity, onClose, onSave, onAI, onWorkspace, onTrack, busy, onAuth }) {
  if (!session) return <LockedState onAuth={onAuth} title="Your shortlist lives here" copy="Save promising opportunities and return to them without searching the full feed again." />;
  return <div><PageTitle eyebrow="Shortlist" title="Keep the opportunities worth a second look." copy="Saved opportunities stay separate from the live feed so you can compare, decide and start preparation when the timing is right."/>
    {items.length ? <div className="opportunity-grid saved-grid">{items.map((row, i) => <OpportunityCard key={row.id} row={row} tone={CARD_TONES[i % CARD_TONES.length]} active={selected?.id === row.id} onClick={() => openOpportunity(row.id)} />)}</div> : <div className="feed-empty"><Bookmark/><h3>Your shortlist is empty</h3><p>Save an opportunity from Discovery and it will appear here.</p></div>}
    <OpportunityPanel row={selected} onClose={onClose} onSave={onSave} onAI={onAI} onWorkspace={onWorkspace} onTrack={onTrack} busy={busy} session={session} onAuth={onAuth} />
  </div>;
}
...
function WorkspaceCanvas({ workspace, readiness, activeDocId, setActiveDocId, generatePlan, saveDocument, aiDraftDocument, aiReviewDocument, addMember, addComment, resolveComment, summarizeTeamReview, finalizeWorkspace, recordSubmission, busy }) {
...
function DocumentEditor({ doc, canEdit, onSave, onDraft, onReview, busy }) {
  ...
  return <div className="document-editor"><div className="editor-head"><div><strong>{doc.title}</strong><small>Version {doc.version} · {doc.generatedByAi ? 'AI drafted' : 'Manual / pending'}</small></div><span className={cx('status-chip', doc.status)}>{statusLabel(doc.status)}</span></div><textarea value={content} onChange={(e) => setContent(e.target.value)} readOnly={!canEdit} placeholder="Draft content here…"/><div className="editor-actions">{canEdit && <><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="pending">Pending</option><option value="drafting">Drafting</option><option value="review">Review</option><option value="approved">Approved</option><option value="complete">Complete</option></select><button className="button ghost" onClick={() => onSave(doc, content, status)}><FileCheck2/> Save version</button><button className="button gold" onClick={() => onDraft(doc)} disabled={busy === `draft-${doc.id}`}><TukuAIIcon size={16}/> AI autofill</button></>}<button className="button ghost" onClick={() => onReview(doc)}><TukuAIIcon size={16}/> AI review</button></div></div>;
}
function TeamPanel({ workspace, canManage, addMember, addComment, resolveComment, summarizeTeamReview }) {
  const [email, setEmail] = useState(''); const [role, setRole] = useState('reviewer'); const [comment, setComment] = useState('');
  const [showResolved, setShowResolved] = useState(false); const [summary, setSummary] = useState(''); const [summaryLoading, setSummaryLoading] = useState(false);
  const comments = (workspace.comments || []).filter((c) => showResolved || c.status !== 'resolved').slice(0, 12);
  async function runSummary() { setSummaryLoading(true); const text = await summarizeTeamReview(); setSummary(text || ''); setSummaryLoading(false); }
  return <section className="surface-panel team-panel">
    <div className="panel-title"><div><UsersRound/><span><strong>Team review</strong><small>{workspace.members?.length || 0} collaborators · {(workspace.comments || []).filter((c)=>c.status!=='resolved').length} open comments</small></span></div><button className="panel-text-button" onClick={runSummary} disabled={summaryLoading}>{summaryLoading ? <LoaderCircle className="spin"/> : <TukuAIIcon size={15}/>} Summarize</button></div>
    {summary && <div className="team-summary"><div><TukuAIIcon size={17}/><strong>Radar review summary</strong></div><p>{summary}</p></div>}
    <div className="member-stack">{(workspace.members || []).map((m) => <div key={m.id}><div className="mini-avatar">{(m.name || m.email || 'R').slice(0,1).toUpperCase()}</div><span><strong>{m.name || m.email}</strong><small>{m.role} · {m.status === 'accepted' ? 'has access' : m.status}</small></span></div>)}</div>
    {canManage && <div className="invite-row"><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="reviewer@example.com"/><select value={role} onChange={(e) => setRole(e.target.value)}><option value="reviewer">Reviewer</option><option value="editor">Editor</option><option value="viewer">Viewer</option></select><button aria-label="Invite collaborator" onClick={() => { if(email.trim()) { addMember(email.trim(), role); setEmail(''); } }}><Plus/></button></div>}
    <div className="comments"><div className="comments-head"><h4>Comments</h4><button onClick={()=>setShowResolved((v)=>!v)}>{showResolved ? 'Hide resolved' : 'Show resolved'}</button></div>{comments.map((c) => <div className={cx('comment', c.status === 'resolved' && 'resolved')} key={c.id}><div className="comment-meta"><strong>{c.authorName || 'Reviewer'}</strong><small>{fmtDateTime(c.createdAt)}</small></div><p>{c.body}</p><button className="comment-status" onClick={()=>resolveComment(c.id, c.status === 'resolved' ? 'open' : 'resolved')}>{c.status === 'resolved' ? 'Reopen' : 'Resolve'}</button></div>)}{comments.length === 0 && <div className="mini-empty">No {showResolved ? '' : 'open '}review comments.</div>}<div className="comment-input"><input value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Add review comment…"/><button aria-label="Add comment" onClick={() => { if(comment.trim()) { addComment(comment.trim()); setComment(''); } }}><Send/></button></div></div>
  </section>;
}

function ApplicationsView({ session, applications, updateApplication, openOpportunity, openWorkspace, onAuth }) {
  if (!session) return <LockedState onAuth={onAuth} title="Track every application outcome" />;
  const submitted = applications.filter((x) => ['applied','interview','offer','rejected','withdrawn'].includes(x.status));
  const interviews = applications.filter((x) => x.status === 'interview'); const offers = applications.filter((x) => x.status === 'offer');
  const columns = [
    ['planning','Planning', (x) => x.status === 'planning'],
    ['applied','Submitted', (x) => x.status === 'applied'],
    ['progress','Progressed', (x) => ['interview','offer'].includes(x.status)],
    ['closed','Closed', (x) => ['rejected','withdrawn'].includes(x.status)],
  ];
  return <div><PageTitle eyebrow="Outcome pipeline" title="Know what is moving." copy="Move applications through the pipeline, reopen the original opportunity, and jump directly into the preparation workspace."/>
    <div className="stats-row"><StatCard icon={BriefcaseBusiness} label="Tracked" value={applications.length} note="pipeline records" tone="navy"/><StatCard icon={Send} label="Submitted" value={submitted.length} note="recorded applications" tone="gold"/><StatCard icon={MessageSquareText} label="Interviews" value={interviews.length} note="progressed" tone="teal"/><StatCard icon={CheckCircle2} label="Offers" value={offers.length} note="positive outcomes" tone="chartreuse"/></div>
    <div className="pipeline-board pipeline-board-four">{columns.map(([key,label,test]) => { const rows = applications.filter(test); return <section key={key}><div className="pipeline-head"><strong>{label}</strong><span>{rows.length}</span></div>{rows.length ? rows.map((x) => <article key={x.id}><span className={cx('status-chip', x.status)}>{statusLabel(x.status)}</span><h3>{x.opportunity?.title || 'Opportunity'}</h3><p>{x.opportunity?.organization || ''}</p><small>{x.notes || (x.opportunity?.deadline ? `Deadline ${fmtDate(x.opportunity.deadline)}` : 'No notes yet.')}</small><div className="application-card-actions"><select aria-label={`Update status for ${x.opportunity?.title || 'application'}`} value={x.status} onChange={(e) => updateApplication(x.opportunity?.id, e.target.value, x.notes || '')}><option value="planning">Planning</option><option value="applied">Submitted</option><option value="interview">Interview</option><option value="offer">Offer</option><option value="rejected">Rejected</option><option value="withdrawn">Withdrawn</option></select><button onClick={() => openOpportunity(x.opportunity?.id)} aria-label="Open opportunity"><ExternalLink/></button>{x.workspaceId && <button onClick={() => openWorkspace(x.workspaceId)} aria-label="Open workspace"><Layers3/></button>}</div></article>) : <div className="pipeline-empty">Nothing here yet.</div>}</section>; })}</div>
  </div>;
}


function DocumentsView({ session, documents, evidenceHealth, addDocument, updateDocument, deleteDocument, extractEvidence, notify, onAuth }) {
  const [open, setOpen] = useState(false); const [editing, setEditing] = useState(null); const [editLoading, setEditLoading] = useState('');
  async function reviewDocument(id) { setEditLoading(id); try { setEditing(await api(`/api/me/documents/${id}`)); } catch (error) { notify(error.message); } finally { setEditLoading(''); } }
  ...
  return <div><PageTitle eyebrow="Reusable evidence" title="Evidence Vault." copy="Keep proof ready, see what your documents actually cover, and close recurring evidence gaps before the next deadline." action={<button className="button dark" onClick={() => setOpen(!open)}><Plus/> Add document</button>} />
    ...
    {open && <DocumentForm onSubmit={async (p) => { await addDocument(p); setOpen(false); }}/>}<div className="document-library">{documents.map((d, i) => <motion.article key={d.id} className={cx('library-card', `tone-${CARD_TONES[i % CARD_TONES.length]}`)} whileHover={{ y: -4 }}><div className="library-icon"><Files/></div><div className="library-top"><span className="status-chip">{d.category}</span><button aria-label={`Delete ${d.title}`} onClick={() => deleteDocument(d.id)}><Trash2/></button></div><h3>{d.title}</h3><p>{d.fileName || 'Stored text evidence'}</p><div className="library-meta"><span>{d.hasContent ? 'AI-ready' : 'Metadata only'}</span><span>{d.expiresAt && new Date(d.expiresAt) < new Date() ? 'Expired' : d.verificationStatus === 'machine_extracted' ? 'Evidence extracted' : d.verificationStatus === 'verified' ? 'Verified' : 'Needs review'}</span>{d.expiresAt && <span>Expires {fmtDate(d.expiresAt)}</span>}</div><div className="library-actions"><button className="text-action" onClick={() => extractEvidence(d.id)}><TukuAIIcon size={15}/> Extract evidence</button><button className="text-action" onClick={() => reviewDocument(d.id)} disabled={editLoading===d.id}>{editLoading===d.id ? <LoaderCircle className="spin"/> : <FileCheck2/>} Review / edit</button></div></motion.article>)}{documents.length === 0 && <div className="empty-grid-card">No evidence documents yet. Add a CV, capability statement, reference, certificate or past proposal to make Radar's eligibility decisions stronger.</div>}</div>
    {editing && <DocumentReviewModal document={editing} onClose={()=>setEditing(null)} onSave={async (payload)=>{ const result=await updateDocument(editing.id,payload); if(result) setEditing(result); }} />}
  </div>;
}
function DocumentReviewModal({ document, onClose, onSave }) {
  const [form,setForm]=useState({ title:document.title||'', category:document.category||'other', content:document.content||'', expiresAt:document.expiresAt ? String(document.expiresAt).slice(0,10) : '' }); const [saving,setSaving]=useState(false);
  useEffect(()=>{setForm({ title:document.title||'', category:document.category||'other', content:document.content||'', expiresAt:document.expiresAt ? String(document.expiresAt).slice(0,10) : '' });},[document.id,document.version]);
  async function save(reviewed=false){setSaving(true);await onSave({...form,expiresAt:form.expiresAt||null,reviewed});setSaving(false);}
  return <div className="modal-layer evidence-modal-layer" role="presentation" onMouseDown={(e)=>{if(e.target===e.currentTarget)onClose();}}><section className="evidence-review-modal" role="dialog" aria-modal="true" aria-label={`Review ${document.title}`}><button className="modal-close" onClick={onClose}><X/></button><span className="eyebrow">Evidence document</span><h2>Review and verify</h2><p>Verification means you have checked that this stored evidence is current and accurate. AI extraction remains machine-generated until you review it.</p><div className="two-fields"><label>Title<input value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})}/></label><label>Category<select value={form.category} onChange={(e)=>setForm({...form,category:e.target.value})}><option value="cv">CV</option><option value="profile">Profile</option><option value="bio">Bio</option><option value="capability">Capability statement</option><option value="reference">Reference</option><option value="certificate">Certificate</option><option value="financial">Financial</option><option value="legal">Legal</option><option value="portfolio">Portfolio</option><option value="other">Other</option></select></label></div><label>Expiry date<input type="date" value={form.expiresAt} onChange={(e)=>setForm({...form,expiresAt:e.target.value})}/></label><label>Extracted / stored text<textarea rows="12" value={form.content} onChange={(e)=>setForm({...form,content:e.target.value})}/></label><div className="review-modal-meta"><span>Status: <strong>{statusLabel(document.verificationStatus)}</strong></span><span>Version {document.version || 1}</span>{document.lastReviewedAt && <span>Reviewed {fmtDate(document.lastReviewedAt)}</span>}</div><div className="review-modal-actions"><button className="button ghost" disabled={saving} onClick={()=>save(false)}>Save changes</button><button className="button gold" disabled={saving} onClick={()=>save(true)}>{saving ? <LoaderCircle className="spin"/> : <ShieldCheck/>} Save & mark verified</button></div></section></div>;
}
...
function DocumentForm({ onSubmit }) {
  const [form, setForm] = useState({ title: '', category: 'cv', content: '', expiresAt: '' }); const [file, setFile] = useState(null); const [fileError, setFileError] = useState('');
  return <form className="document-form" onSubmit={async (e) => { e.preventDefault(); if (fileError) return; const payload = { ...form, expiresAt: form.expiresAt || null }; if (file) { payload.fileName = file.name; payload.mimeType = file.type || 'application/pdf'; payload.base64 = await fileToBase64(file); } await onSubmit(payload); }}><label>Title<input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Master CV 2026"/></label><label>Category<select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}><option value="cv">CV</option><option value="profile">Profile</option><option value="bio">Bio</option><option value="capability">Capability statement</option><option value="reference">Reference</option><option value="certificate">Certificate</option><option value="financial">Financial</option><option value="legal">Legal</option><option value="portfolio">Portfolio</option><option value="other">Other</option></select></label><label>Expiry date <small className="field-help">Optional</small><input type="date" value={form.expiresAt} onChange={(e) => setForm({ ...form, expiresAt: e.target.value })}/></label><label className="wide">Upload<input type="file" accept="application/pdf,text/plain,.pdf,.txt" onChange={(e) => { const next=e.target.files?.[0] || null; if (next && next.size > 5 * 1024 * 1024) { setFile(null); setFileError('Files must be 5 MB or smaller.'); } else { setFile(next); setFileError(''); } }}/>{fileError && <small className="form-error">{fileError}</small>}</label><label className="wide">Or paste text<textarea rows="5" value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })}/></label><button className="button gold" disabled={Boolean(fileError)}>Save to library</button></form>;
}
...
function AnalyticsView({ session, analytics, onAuth }) {
  ...
  return <div><PageTitle eyebrow="Performance analytics" title="See what converts." copy="Measure opportunity quality, preparation throughput and outcomes—not just how many listings Radar found."/><div className="analytics-grid">{metrics.map(([l,v,p],i) => <article key={l} className={cx('analytics-card', `tone-${CARD_TONES[i % CARD_TONES.length]}`)}><span>{l}</span><strong>{v}</strong><p>{p}</p></article>)}</div><section className="surface-panel analytics-panel"><div className="panel-title"><div><ChartNoAxesCombined/><span><strong>Application pipeline distribution</strong><small>Current tracked outcomes</small></span></div></div><div className="bar-chart">{Object.keys(statuses).length === 0 && <div className="mini-empty">Track an application to start seeing outcome analytics.</div>}{Object.entries(statuses).map(([k,v]) => <div key={k}><span>{statusLabel(k)}</span><div><i style={{ width: `${Math.round((Number(v)/max)*100)}%` }}/></div><strong>{v}</strong></div>)}</div></section></div>;
}

function ProfileView({ session, profile, capability, briefing, briefPreview, briefPreviewLoading, previewBriefing, notifications, unread, markAllRead, saveProfile, saveCapability, saveBriefing, uploadResume, onAuth }) {
  ...
  return <div><PageTitle eyebrow="Matching context" title="Make Radar know you." copy="Better profile evidence means sharper ranking, fewer false positives and more grounded AI drafting."/><div className="profile-layout"><ProfileForm initial={profile} onSave={saveProfile} uploadResume={uploadResume}/><CapabilityForm initial={capability} onSave={saveCapability}/><BriefingCard initial={briefing} preview={briefPreview} previewLoading={briefPreviewLoading} onPreview={previewBriefing} onSave={saveBriefing}/><section className="settings-card notification-card"><div className="settings-head"><div><Bell/><span><strong>Notifications</strong><small>{unread} unread</small></span></div><button onClick={markAllRead}>Mark all read</button></div><div className="notification-list">{notifications.slice(0,8).map((n) => <article key={n.id} className={!n.readAt ? 'unread' : ''}><span/><div><strong>{n.title}</strong><p>{n.body}</p><small>{fmtDateTime(n.createdAt)}</small></div></article>)}{notifications.length === 0 && <div className="mini-empty">No Radar activity yet.</div>}</div></section></div></div>;
}
...
function CapabilityForm({ initial, onSave }) {
  ...
  return <form className="settings-card" onSubmit={(e) => { e.preventDefault(); onSave({ ...f, yearsOperating: f.yearsOperating, sectors: list(f.sectors), countries: list(f.countries), donorExperience: list(f.donorExperience), licences: list(f.licences), referenceCount: Number(f.referenceCount || 0) }); }}><div className="settings-head"><div><ShieldCheck/><span><strong>Capability & credentials</strong><small>Eligibility intelligence</small></span></div></div><div className="two-fields"><label>Capability represents<select value={f.profileType} onChange={(e)=>setF({...f,profileType:e.target.value})}><option value="individual">Individual</option><option value="firm">Firm / organisation</option><option value="both">Both</option></select></label><label>Past references available<input type="number" min="0" value={f.referenceCount} onChange={(e)=>setF({...f,referenceCount:e.target.value})}/></label></div><div className="two-fields"><label>Legal / trading name<input value={f.legalName} onChange={(e)=>setF({...f,legalName:e.target.value})}/></label><label>Registration country<input value={f.registrationCountry} onChange={(e)=>setF({...f,registrationCountry:e.target.value})}/></label></div><div className="two-fields"><label>Registration number<input value={f.registrationNumber} onChange={(e)=>setF({...f,registrationNumber:e.target.value})}/></label><label>Years operating<input type="number" value={f.yearsOperating} onChange={(e)=>setF({...f,yearsOperating:e.target.value})}/></label></div><label>Turnover / capacity band<input value={f.turnoverBand} onChange={(e)=>setF({...f,turnoverBand:e.target.value})}/></label><label>Sectors<textarea rows="3" value={f.sectors} onChange={(e)=>setF({...f,sectors:e.target.value})}/></label><label>Countries of operation<textarea rows="2" value={f.countries} onChange={(e)=>setF({...f,countries:e.target.value})}/></label><label>Donor / institutional experience<textarea rows="3" value={f.donorExperience} onChange={(e)=>setF({...f,donorExperience:e.target.value})}/></label><label>Licences / certifications<textarea rows="2" value={f.licences} onChange={(e)=>setF({...f,licences:e.target.value})}/></label><label className="switchline"><input type="checkbox" checked={f.canRecruitSpecialists} onChange={(e)=>setF({...f,canRecruitSpecialists:e.target.checked})}/><span>I can recruit specialists to close technical gaps</span></label><button className="button gold">Save capability profile</button></form>;
}
function BriefingCard({ initial, preview, previewLoading, onPreview, onSave }) {
...
function AuthModal({ open, setOpen, mode, setMode, recoveryToken, resetRequested, onResetComplete, onSuccess, notify }) {
  ...
  async function submit(e) { e.preventDefault(); setLoading(true); try {
    if (resetRequested) {
      if (!recoveryToken) throw new Error('This password-reset link is incomplete. Request a fresh reset email.');
      await api('/api/auth/reset-password',{method:'POST',body:JSON.stringify({recoveryToken,password})}); notify('Password updated. Sign in with your new password.'); setPassword(''); onResetComplete();
    } else if (forgot) { await api('/api/auth/forgot-password',{method:'POST',body:JSON.stringify({email})}); notify('Password reset instructions sent if the account exists.'); setForgot(false); }
    else { const result = await api('/api/auth/credentials',{method:'POST',body:JSON.stringify({mode,email,password,name})}); if (result.verificationRequired) notify('Check your email to verify the account.'); else await onSuccess(); }
  } catch(error){ notify(error.message); } finally { setLoading(false); } }
  const resetMode = resetRequested;
  return <AnimatePresence>{open && <motion.div className="modal-layer" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><motion.div className="auth-modal" role="dialog" aria-modal="true" aria-label="Radar account" initial={{scale:.96,y:20,opacity:0}} animate={{scale:1,y:0,opacity:1}} exit={{scale:.98,y:10,opacity:0}}><button className="modal-close" onClick={()=>setOpen(false)}><X/></button><div className="auth-brand"><div className="radar-mark"><span/><span/><i/></div><strong>RADAR</strong></div><span className="eyebrow">Tuku account</span><h2>{resetMode ? 'Choose a new password.' : forgot ? 'Reset your password' : mode==='signup' ? 'Start your 30-day trial.' : 'Welcome back.'}</h2><p>{resetMode ? (recoveryToken ? 'Use at least 8 characters. The recovery token is handled securely in this session.' : 'This link is missing its recovery token. Request a fresh reset email below.') : forgot ? 'We will send a secure reset link through Tuku Auth.' : 'One account for Radar and the wider Tuku product estate.'}</p><form onSubmit={submit}>{!resetMode && !forgot && mode==='signup' && <label>Name<input required value={name} onChange={(e)=>setName(e.target.value)} placeholder="Your name"/></label>}{!resetMode && <label>Email<input type="email" required value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="you@example.com"/></label>}{!forgot && <label>{resetMode ? 'New password' : 'Password'}<input type="password" minLength="8" required value={password} onChange={(e)=>setPassword(e.target.value)} placeholder="At least 8 characters"/></label>}<button className="button gold" disabled={loading || (resetMode && !recoveryToken)}>{loading ? <LoaderCircle className="spin"/> : null}{resetMode ? 'Update password' : forgot ? 'Send reset link' : mode==='signup' ? 'Create account' : 'Sign in'}</button></form>{resetMode && !recoveryToken ? <button className="auth-link" onClick={()=>{onResetComplete();setForgot(true);setOpen(true);}}>Request a fresh reset email</button> : !resetMode && !forgot ? <button className="auth-link" onClick={()=>setForgot(true)}>Forgot password?</button> : null}{!resetMode && (forgot ? <button className="auth-link" onClick={()=>setForgot(false)}>Back to sign in</button> : <button className="auth-link" onClick={()=>setMode(mode==='signup'?'signin':'signup')}>{mode==='signup'?'Already have an account? Sign in':'New to Radar? Create account'}</button>)}</motion.div></motion.div>}</AnimatePresence>;
}

function AIDrawer({ open, setOpen, messages, onAsk, context }) {
  const [message,setMessage] = useState('');
  return <AnimatePresence>{open && <><motion.button className="drawer-scrim" onClick={()=>setOpen(false)} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}/><motion.aside className="ai-drawer" role="dialog" aria-modal="true" aria-label="Radar AI" initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} transition={spring}><div className="drawer-head"><div><span className="eyebrow">Private intelligence</span><h2><TukuAIIcon size={20}/> Ask Radar AI</h2></div><button onClick={()=>setOpen(false)}><X/></button></div><div className="ai-context"><Target/><span>{context}</span></div><div className="chat-stream">{messages.map((m,i)=><div key={i} className={cx('chat-bubble',m.role,m.loading&&'loading')}>{m.loading && <LoaderCircle className="spin"/>}{m.text}</div>)}</div><form className="chat-form" onSubmit={(e)=>{e.preventDefault();const text=message.trim();if(!text)return;setMessage('');onAsk(text)}}><textarea rows="3" value={message} onChange={(e)=>setMessage(e.target.value)} placeholder="What should I do next?"/><button className="button gold"><TukuAIIcon size={16}/> Ask Radar</button></form></motion.aside></>}</AnimatePresence>;
}
